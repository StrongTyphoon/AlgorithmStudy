class MaxHeap {
  ...
  insert(node) {
    ...
    while (current > 1) {
      ...
      if (this.heap[parent].get('key') < node.get('key')) {
  ...
  remove() {
    ...
    if (this.heap.length > 2) {
      ...
      while (this.heap[leftChildIndex]) {
        ...
        if (this.heap[current].get('key') < this.heap[childIndexToCompare].get('key')) {
...
const key = [1,5,6,7,2,3]
const val = [11,55,76,87,92,13]
